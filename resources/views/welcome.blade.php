...
<body class="antialiased bg-gray-900 text-white min-h-screen flex flex-col">
  ...
  <main class="relative isolate flex-1 overflow-hidden">
    <!-- ====== HERO CAROUSEL ====== -->
    <section id="hero" class="relative h-[70vh] sm:h-[80vh] md:h-[85vh] lg:h-screen">
      <div id="carousel" class="relative h-full overflow-hidden" aria-roledescription="carousel" aria-label="Promos Rentacar" tabindex="0">
        ...
        <div id="carouselTrack" class="carousel-track flex h-full">
          ...
          <div class="relative w-full shrink-0 h-full">
            ...
            <div class="absolute inset-0 flex items-start bg-black/40 sm:bg-black/30 pt-16 sm:pt-20 md:pt-24 lg:pt-32">
              <div class="px-4 sm:px-6 lg:px-24 max-w-3xl">
                <h1 class="text-2xl sm:text-3xl md:text-4xl lg:text-5xl font-extrabold leading-tight">Movilidad sin límites</h1>
                <p class="mt-2 sm:mt-3 text-sm sm:text-base text-gray-200 leading-relaxed">
          ...
          <div class="relative w-full shrink-0 h-full">
            ...
            <div class="absolute inset-0 flex items-start bg-black/40 sm:bg-black/30 pt-16 sm:pt-20 md:pt-24 lg:pt-32">
              <div class="px-4 sm:px-6 lg:px-24 max-w-3xl">
                <h2 class="text-2xl sm:text-3xl md:text-4xl lg:text-5xl font-extrabold leading-tight">Planes para empresas</h2>
                <p class="mt-2 sm:mt-3 text-sm sm:text-base text-gray-200 leading-relaxed">
          ...
          <div class="relative w-full shrink-0 h-full">
            ...
            <div class="absolute inset-0 flex items-start bg-black/40 sm:bg-black/30 pt-16 sm:pt-20 md:pt-24 lg:pt-32">
              <div class="px-4 sm:px-6 lg:px-24 max-w-3xl">
                <h2 class="text-2xl sm:text-3xl md:text-4xl lg:text-5xl font-extrabold leading-tight">Reserva en minutos</h2>
                <p class="mt-2 sm:mt-3 text-sm sm:text-base text-gray-200 leading-relaxed">
          ...
          <div class="relative w-full shrink-0 h-full">
            ...
            <div class="absolute inset-0 flex items-start bg-black/40 sm:bg-black/30 pt-16 sm:pt-20 md:pt-24 lg:pt-32">
              <div class="px-4 sm:px-6 lg:px-24 max-w-3xl">
                <h2 class="text-2xl sm:text-3xl md:text-4xl lg:text-5xl font-extrabold leading-tight">Atención que te acompaña</h2>
                <p class="mt-2 sm:mt-3 text-sm sm:text-base text-gray-200 leading-relaxed">
        ...
        <div id="indicators" class="absolute bottom-4 sm:bottom-6 left-4 sm:left-6 z-20 flex gap-2 sm:gap-3 items-center justify-start">
          <button class="dot w-2.5 h-2.5 sm:w-3 sm:h-3 rounded-full bg-white/60 cursor-pointer" data-slide="0" aria-label="Ir al slide 1"></button>
          <button class="dot w-2.5 h-2.5 sm:w-3 sm:h-3 rounded-full bg-white/40 cursor-pointer" data-slide="1" aria-label="Ir al slide 2"></button>
          <button class="dot w-2.5 h-2.5 sm:w-3 sm:h-3 rounded-full bg-white/40 cursor-pointer" data-slide="2" aria-label="Ir al slide 3"></button>
          <button class="dot w-2.5 h-2.5 sm:w-3 sm:h-3 rounded-full bg-white/40 cursor-pointer" data-slide="3" aria-label="Ir al slide 4"></button>
        </div>
      </div>
    </section>

    <!-- ====== LOGIN DESKTOP (lateral derecho) ====== -->
    <div class="hidden lg:flex absolute inset-0 items-center justify-end px-6 lg:px-24 pointer-events-none">
      <div class="w-full max-w-sm pr-6 pointer-events-auto">
        <div class="bg-gray-800/70 backdrop-blur p-8 lg:p-12 rounded-2xl shadow-2xl border border-white/10 scale-[1.05]">
          <h2 class="text-2xl lg:text-3xl font-bold text-white mb-6 lg:mb-8 text-center">Iniciar Sesión</h2>
          <form action="{{ route('sendLogin') }}" method="POST" class="space-y-5 lg:space-y-6">
            @csrf
            @error('errorUser') <p class="text-red-400 text-xs sm:text-sm">{{ $message }}</p> @enderror
            @error('errorCred') <p class="text-red-400 text-xs sm:text-sm">{{ $message }}</p> @enderror
            ...
            <div>
              ...
              <input type="text" name="username" value="{{ old('username') }}" placeholder="Usuario"
                     class="w-full px-3 py-2.5 lg:py-3 rounded bg-gray-700 text-white focus:outline-none focus:ring-2 focus:ring-blue-700 text-sm lg:text-base" required>
            </div>
            <div>
            ...
            <!-- Botón azul marino -->
            <button type="submit" class="w-full py-2.5 lg:py-3 rounded-md bg-blue-900 hover:bg-blue-800 font-semibold text-white transition text-sm lg:text-base">
              Entrar
            </button>
          </form>

          <!-- Enlace azul marino -->
          <p class="mt-5 lg:mt-6 text-xs sm:text-sm text-gray-300 text-center">
            ¿No tienes cuenta?
    ...
    <!-- ====== LOGIN MOBILE (abajo del carrusel) ====== -->
    <div class="lg:hidden w-full bg-gray-900/95 backdrop-blur-sm border-t border-white/10">
      <div class="max-w-md mx-auto px-4 sm:px-6 py-6 sm:py-8">
        <h2 class="text-2xl font-bold text-white mb-6 text-center">Iniciar Sesión</h2>
        <form action="{{ route('sendLogin') }}" method="POST" class="space-y-5">
          @csrf
